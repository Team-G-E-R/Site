...
export function LoginModal({ onClose }: { onClose: () => void }) {
  ...
  const submit = async () => {
    ...
    try { if (tab === 'login') await login(email, password); else await register(email, password, name); onClose(); }
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="w-full max-w-md bg-[#0f131a] border border-white/10 rounded-2xl p-6 shadow-[0_10px_30px_rgba(0,0,0,0.6)]">
        ...
        <div className="mt-4 text-xs text-white/50">
          ...
          {getBackendMode() === 'http' && <> · API: <span className="text-white/80">{getApiBase()}</span></>}
        </div>
      </div>
    </div>
  );
